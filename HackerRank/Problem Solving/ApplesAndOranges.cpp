...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) 
{
    ...
    vector<int> updatedApples, updatedOranges;
    
    for(auto c : apples)
    {
        updatedApples.push_back(a+c);
    }
    
    for(auto c : oranges)
    {
        updatedOranges.push_back(b+c);
    }
    
    uint aCount = 0, oCount = 0;
    
    for(auto c : updatedApples)
    {
        if(c >= s)
        {
            if(c <= t)
